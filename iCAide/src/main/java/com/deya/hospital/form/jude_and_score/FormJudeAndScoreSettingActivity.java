...
public class FormJudeAndScoreSettingActivity extends BaseActivity implements
		OnClickListener {
	...
	TextView typetext;
	FormJugeAndSocreEditorAdapter adapter2;
	...
	private void intiView() {
		...
		// if(formtype == 1){
		// adapter = new FormEditorAdapter2(mcontext,isUpdated,editorbal,list,
		// new TextContral() {
		//
		// @Override
		...
		adapter2 = new FormJugeAndSocreEditorAdapter(mcontext, isUpdated,
				editorbal, list, itemList,0, new FormJudeAndScoreSettingInter() {

					@Override
					public void onDelResult(int postion) {// 点击对错事件

					}

					@Override
					public void onDecreaseScore(double position) {// 减分

					}

					@Override
					public void onAddScore(double position) {// 加分
