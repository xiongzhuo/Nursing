...
public class PreViewActivityDetial extends BaseActivity implements
		OnClickListener {
	ListView listView;
...
private TextView departTv;
	...
	private void initView() {
		...
		if (type == 1||type==4) {
			...
			totalScoreTv2.setText("质量标准" + "(总分："
					+ AbStrUtil.reMoveUnUseNumber(data.getScore() + "") + ")");
		} else {
		...
		adapter = new PreviewDitialAdapter(mcontext, list, type,new AdpterInter() {
	...
	private void getFormDitail() {
		...
		MainBizImpl.getInstance().onComomRequest(myHandler,
				PreViewActivityDetial.this, GET_SUCESS, GET_FAILE,
				job,"grid/queryGridTaskDetailByTaskId");
	}
