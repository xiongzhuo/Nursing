 ...
 * @date ${date}
 */
public class CaseFormUpload extends BaseUploader {
    @Override
    ...
    private boolean UploadTask(TaskVo taskVo) {
        ...
        RisistantVo.TaskInfoBean taskInfoBean = gson.fromJson(taskVo.getTaskListBean(),  RisistantVo.TaskInfoBean.class);
        CaseListVo.CaseListBean businessBean=taskInfoBean.getBaseInfo();
        try {
            job.put("authent", MyAppliaction.getConstantValue(Constants.AUTHENT));
            job.put("task_id", taskVo.getTask_id());
            job.put("tempId",taskInfoBean.getTempId());
            ...
            if(AbStrUtil.isEmpty(supervisorJs.optString("check_content"))){
                supervisorJs.put("check_content","未填写");
            }
            job.put("supervisor_question", supervisorJs);

            JSONArray jarr = json.optJSONArray("resultlist");
            Log.i("upload",job.toString());
            job.put("temp_result", jarr);
            JSONObject map = MainBizImpl.getInstance().sendSyncRequest("quality/submit", job);

            Log.i("upload",map.toString());
