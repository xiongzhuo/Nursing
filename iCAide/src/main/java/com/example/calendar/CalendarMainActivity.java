...
import com.example.calendar.manager.Formatter;
import com.example.calendar.widget.CalendarLinearLayoutWithListView;
...
public class CalendarMainActivity extends BaseActivity implements
    ...
    AnimationDrawable falshdrawable;
    ImageView falshImg;
    String taskType = "0";
...
//		lay_add1.setOnClickListener(this);
    ...
    private void initCalendarManager() {
        manager = new CalendarManager(LocalDate.now(),
                CalendarManager.State.WEEK, LocalDate.now().minusYears(2),
    ...
    private void setMonthkData(List<TaskVo> list) {
        MyAppliaction.monthToatalList.clear();
        MyAppliaction. monthToatalList.addAll(list);
        MyAppliaction. mMonthTaskNums.clear();
        for (int i = 0; i <MyAppliaction. monthToatalList.size(); i++) {
            TaskVo taskVo =MyAppliaction. monthToatalList.get(i);
            ...
            Integer num = MyAppliaction.mMonthTaskNums.get(str);
            if (null != num) {
                MyAppliaction. mMonthTaskNums.put(str, num + 1);
            } else {
                MyAppliaction.mMonthTaskNums.put(str, 1);
            }
        }
        // refreshViewPager(false);
    }
     ...
     * 刷新日历视图
     *
     * @param isAll true：更新viewpager中所有子日历；false：仅更新当前可见日历
     */
    ...
    private void initListView() {
        ...
        planLv.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view,
                ...
                TaskUtils.onStaractivity(CalendarMainActivity.this, tv,
                        dayTaskLi);
            }
        });
    ...
    private void setCalenderText() {
        MyAppliaction.unDoTastNums.clear();
        for (TaskVo taskVo : taskList) {
            ...
            Integer num2 = MyAppliaction.unDoTastNums.get(str);
            if (statu == 2 || statu == 3) {
                MyAppliaction.unDoTastNums.put(str, null != num2 ? num2 + 1 : 1);
            } else if (null == num2) {
                MyAppliaction.unDoTastNums.put(str, 0);
            }
            Log.i("444444444", str + "=============" + MyAppliaction.unDoTastNums.get(str)
                    + "");
        }
    }
    ...
    // 设置当前数据，服务器数据+本地数据
    List<TaskVo> dayTaskLi = new ArrayList<>();
