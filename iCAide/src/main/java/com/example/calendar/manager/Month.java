...
public class Month extends RangeUnit {
    ...
    protected Month(@NonNull LocalDate date, @NonNull LocalDate today,
            @Nullable LocalDate minDate, @Nullable LocalDate maxDate) {
        super(date.withDayOfMonth(1),
                date.withDayOfMonth(date.dayOfMonth().getMaximumValue()), today,
                minDate, maxDate);
    ...
    public boolean next() {
        ...
        if (hasNext()) {
            ...
            setTo(getFrom()
                    .withDayOfMonth(getFrom().dayOfMonth().getMaximumValue()));
    ...
    LocalDate getFirstDateOfCurrentMonth(@NonNull LocalDate currentMonth) {
        ...
        if (currentMonth != null) {
            ...
            if (year == fromYear && month == fromMonth) { return from; }
        }
