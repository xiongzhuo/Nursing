...
public class ResizeManager {
    ...
    public boolean onTouchEvent(@NonNull MotionEvent event) {
        ...
        if (null == mVelocityTracker) { return false; }
        if (action == MotionEvent.ACTION_MOVE) {
            mVelocityTracker.addMovement(event);
        }
        
        if (mState == State.DRAGGING) {
            if (null != mProgressManager && mProgressManager.isPreDrawed()) {
                switch (action) {
                case MotionEvent.ACTION_MOVE:
    ...
    private boolean onDownEvent(@NonNull MotionEvent event) {
        if (MotionEventCompat.getActionMasked(
                event) != MotionEvent.ACTION_DOWN) { throw new IllegalStateException(
                        "Has to be down event!"); }
        ...
        if (!mScroller.isFinished()) {
            ...
            if (mScroller.getFinalY() == 0) {
                mDragStartY = mDownY + mScroller.getStartY()
                        - mScroller.getCurrY();
            } else {
    ...
    //返回false 可以左右滑动；
    public boolean checkForResizing(MotionEvent ev) { // FIXME this method
                                                      // should only return true
                                                      // false. Make another
                                                      // method for starting
                                                      // animation
    	
        if (mState == State.DRAGGING) { return true; }
        ...
        if ((yDIff > 0 && state == CalendarManager.State.MONTH) || (yDIff < 0
                && state == CalendarManager.State.WEEK)) { return false; }
        
        if (Math.abs(yDIff) > mTouchSlop) { // FIXME this should happen only
                                            // if dragging int right
                                            // direction
            mState = State.DRAGGING;
            mDragStartY = ev.getY();
            ...
            if (mProgressManager == null) {
                ...
                int weekOfMonth = manager.getSelectedWeekOfMonth(
                        mCalendarView.getCurrentMonth());
               
                
                if (state == CalendarManager.State.WEEK) { // always animate in
                                                           // month view
                    manager.toggleView(mCalendarView);
                    mCalendarView.populateLayout();
                }
              
                mProgressManager = new ProgressManagerImpl(mCalendarView,
                        weekOfMonth, state == CalendarManager.State.MONTH);
                
            }
            DebugUtil.debug("checkForResizing", "state>>"+state+"  yDIff》》"+yDIff+" ");
            return true;
        }else{
        	
        	 DebugUtil.debug("checkForResizing", "state>>"+state);
        }
        return false;
    }
    ...
    public void onDraw() {
        ...
        if (!mScroller.isFinished()) {
            ...
            float position = mScroller.getCurrY() * 1f
                    / mProgressManager.getEndSize();
            mProgressManager.apply(position);
            mCalendarView.postInvalidate();
            DebugUtil.debug("pull_pos", "isFinished() position>"+position);
        	...
        	DebugUtil.debug("mProgressManager.finish", "position>>"+ mScroller.getCurrY() +" "+mProgressManager.getEndSize());
