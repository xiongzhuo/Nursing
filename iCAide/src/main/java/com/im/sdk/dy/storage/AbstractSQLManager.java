...
public abstract class AbstractSQLManager {
	...
	public AbstractSQLManager() {
		openDatabase(MyAppliaction.getmInstance(),
				WebUrl.getDataVersionCode());
	}

	private void openDatabase(Context context, int databaseVersion) {
		if (databaseHelper == null&&CCPAppManager.getClientUser()!=null) {
			databaseHelper = new DatabaseHelper(context, this, databaseVersion);
		}
		// DatabaseManager.initializeInstance(databaseHelper);
		if (sqliteDB == null&&databaseHelper!=null) {
	...
	private void open(boolean isReadonly) {
		if (sqliteDB == null&&null!=databaseHelper) {
			if (isReadonly) {
	...
	static class DatabaseHelper extends SQLiteOpenHelper {
		...
		/** 数据库名称 */
		static final String DATABASE_NAME = WebUrl.APP_CODE+"_im.db";
		...
		static final String TABLES_NAME_IM_MESSAGE_HISTORY = "im_message_hivstory";

		private AbstractSQLManager mAbstractSQLManager;

		public DatabaseHelper(Context context, AbstractSQLManager manager,
				int version) {
			 this(context, manager, WebUrl.getInstance().isTest?( getBaseSDPath()+
			 CCPAppManager.getClientUser().getUserId() + "_" + DATABASE_NAME):(CCPAppManager.getClientUser().getUserId()
						+ "_" + DATABASE_NAME),
			 null, version);
			 DebugUtil.debug("DatabaseHelper","phone>>"+CCPAppManager.getClientUser().getUserId());
		}
		...
		private void onAddHistoryTable(SQLiteDatabase db) {
			String sql = "CREATE TABLE IF NOT EXISTS "
					+ TABLES_NAME_IM_MESSAGE_HISTORY + " (" + IMessageColumn.ID
					+ " INTEGER PRIMARY KEY AUTOINCREMENT, "
					+ IMessageColumn.MESSAGE_ID
					+ " TEXT UNIQUE ON CONFLICT ABORT, "
					+ IMessageColumn.OWN_THREAD_ID + " INTEGER, "
					+ IMessageColumn.CREATE_DATE + " TEXT, "
					+ IMessageColumn.RECEIVE_DATE + " TEXT, "
					+ IMessageColumn.sender + " TEXT, " + IMessageColumn.BODY
					+ " TEXT, " + IMessageColumn.USER_DATA + " TEXT, "
		...
		void createTableForContacts(SQLiteDatabase db) {
			...
			String sql = "CREATE TABLE IF NOT EXISTS " + TABLES_NAME_CONTACT
					...
					+ ContactsColumn.SEX + " INTEGER, "
					+ ContactsColumn.HOSPITAL + " TEXT, "
					+ ContactsColumn.PROVINCE + " TEXT, " 
					+ ContactsColumn.CITY
					+ " TEXT, " + ContactsColumn.DEPARTMENT + " TEXT, "
					+ ContactsColumn.RNAME + " TEXT ," 
					+ ContactsColumn.OTHER_STR + " TEXT ," 
					+ ContactsColumn.OTHER_STR2 + " TEXT ," 
					+ ContactsColumn.OTHER_INT + " INTEGER " 
		...
		void createTableForIMessage(SQLiteDatabase db) {
			...
			String sql = "CREATE TABLE IF NOT EXISTS " + TABLES_NAME_IM_MESSAGE
					...
					+ IMessageColumn.VERSION + "  INTEGER DEFAULT 0, "
					+ IMessageColumn.DURATION + "  INTEGER DEFAULT 0," 
					+ IMessageColumn.OTHER_STR + " TEXT ," 
					+ IMessageColumn.OTHER_STR2 + " TEXT ," 
					+ IMessageColumn.OTHER_INT + " INTEGER ,"+IMessageColumn.TASK_ID+ " INTEGER "
		...
		void createTableForISession(SQLiteDatabase db) {
			...
			String sql = "CREATE TABLE IF NOT EXISTS " + TABLES_NAME_IM_SESSION
					+ " (" + IThreadColumn.ID
					+ " INTEGER PRIMARY KEY AUTOINCREMENT, "
					+ IThreadColumn.THREAD_ID + " TEXT, "
					+ IThreadColumn.CONTACT_ID + " TEXT , "
					+ IThreadColumn.UNREAD_COUNT + " INTEGER DEFAULT 0, "
					+ IThreadColumn.SNIPPET + "  TEXT, " + IThreadColumn.DATE
					+ "  TEXT, " + IThreadColumn.BOX_TYPE
					+ " INTEGER DEFAULT 0, " + IThreadColumn.SEND_STATUS
					+ " INTEGER DEFAULT 0, " + IThreadColumn.MESSAGE_TYPE
					+ " INTEGER DEFAULT 0, " + IThreadColumn.MESSAGE_COUNT
					+ " INTEGER DEFAULT 0,"
					+ IThreadColumn.OTHER_STR + " TEXT ," 
					+ IThreadColumn.OTHER_STR2 + " TEXT ," 
					+ IThreadColumn.OTHER_INT + " INTEGER " 
		 ...
		 * @param db
		 */
		void createTaleForECGroups(SQLiteDatabase db) {
			String sql2 = "CREATE TABLE IF NOT EXISTS " + TABLES_NAME_GROUPS_2
					+ " (" + GroupColumn.ID
					+ " INTEGER PRIMARY KEY AUTOINCREMENT, "
					...
					+ GroupColumn.GROUP_isdefault + " INTEGER DEFAULT 0, "
					+ GroupColumn.GROUP_DATE_CREATED + "  TEXT, "
					+ GroupColumn.GROUP_Discussion + "  INTEGER DEFAULT 0,"
						+ GroupColumn.OTHER_STR + " TEXT ," 
					+ GroupColumn.OTHER_STR2 + " TEXT ," 
					+ GroupColumn.OTHER_INT + " INTEGER " 
					+ ")";
			LogUtil.v(TAG + ":" + sql2);
			db.execSQL(sql2);
		}
		...
		void createTableGroupMembers(SQLiteDatabase db) {
			String sql = "CREATE TABLE IF NOT EXISTS "
					+ TABLES_NAME_GROUP_MEMBERS + " (" + GroupMembersColumn.ID
					+ " INTEGER PRIMARY KEY AUTOINCREMENT, "
					...
					+ GroupMembersColumn.OTHER_STR + " TEXT ," 
					+ GroupMembersColumn.OTHER_STR2 + " TEXT ," 
					+ GroupMembersColumn.OTHER_INT + " INTEGER " 
					+ ")";
			LogUtil.v(TAG + ":" + sql);
			db.execSQL(sql);
		}
		...
		void createTableSystemNotice(SQLiteDatabase db) {
			String sql = "CREATE TABLE IF NOT EXISTS "
					...
					+ SystemNoticeColumn.NOTICE_WHO + " TEXT ,"
						+ SystemNoticeColumn.OTHER_STR + " TEXT ," 
					+ SystemNoticeColumn.OTHER_STR2 + " TEXT ," 
					+ SystemNoticeColumn.OTHER_INT + " INTEGER " 
					+ ")";
			LogUtil.v(TAG + ":" + sql);
			db.execSQL(sql);
		}
		...
		void createTriggerForIMessage(SQLiteDatabase db) {
			...
			String sql = "CREATE TRIGGER IF NOT EXISTS delete_obsolete_threads_im AFTER DELETE ON "
					+ TABLES_NAME_IM_MESSAGE
					+ " BEGIN   "
					+ " DELETE FROM "
					+ TABLES_NAME_IM_SESSION
					+ " WHERE id = old."
					+ IMessageColumn.OWN_THREAD_ID
					+ " AND id NOT IN ( SELECT "
					+ IMessageColumn.OWN_THREAD_ID
					+ " FROM "
					+ TABLES_NAME_IM_MESSAGE + ");" + " END;";
			LogUtil.d(LogUtil.getLogUtilsTag(AbstractSQLManager.class), sql);
			db.execSQL(sql);

			sql = "CREATE TRIGGER IF NOT EXISTS im_update_thread_on_delete AFTER DELETE ON "
					+ TABLES_NAME_IM_MESSAGE
					+ " "
					+ "BEGIN   "
					+ "UPDATE "
					...
					+ TABLES_NAME_IM_SESSION
					+ " SET "
					+ IThreadColumn.SNIPPET
					+ " = (SELECT "
					+ IThreadColumn.SNIPPET
					+ " FROM (SELECT "
					+ IMessageColumn.RECEIVE_DATE
					+ ", "
					+ IMessageColumn.BODY
					+ " AS "
					+ IThreadColumn.SNIPPET
					...
					+ TABLES_NAME_IM_SESSION
					+ " SET "
					+ IThreadColumn.MESSAGE_TYPE
					+ " = (SELECT "
					+ IMessageColumn.MESSAGE_TYPE
					+ " FROM (SELECT "
					+ IMessageColumn.RECEIVE_DATE
					+ ", "
					+ IMessageColumn.MESSAGE_TYPE
			...
			LogUtil.d(LogUtil.getLogUtilsTag(AbstractSQLManager.class), sql);
			db.execSQL(sql);
			sql = "CREATE TRIGGER IF NOT EXISTS im_update_thread_on_delete2 AFTER DELETE ON "
					+ TABLES_NAME_IM_MESSAGE
					+ " "
					+ "BEGIN   "
					+ "UPDATE "
					+ TABLES_NAME_IM_SESSION
					+ " SET "
					+ IThreadColumn.CONTACT_ID
					+ " = (SELECT "
					+ IMessageColumn.sender
					+ " FROM (SELECT "
					+ IMessageColumn.sender
					+ ", "
					+ IMessageColumn.CREATE_DATE
					...
					+ TABLES_NAME_IM_SESSION
					+ ".id = old."
					+ IMessageColumn.OWN_THREAD_ID + ";   " + "END;";
			LogUtil.d(LogUtil.getLogUtilsTag(AbstractSQLManager.class), sql);
			db.execSQL(sql);

			sql = "CREATE TRIGGER IF NOT EXISTS im_update_thread_on_insert AFTER INSERT ON "
					+ TABLES_NAME_IM_MESSAGE
					+ " "
					+ "BEGIN  "
					+ "UPDATE "
					...
					+ IMessageColumn.OWN_THREAD_ID
					+ " AND new."
					+ IMessageColumn.BOX_TYPE + " == 1 ;" + "END;";
			LogUtil.d(LogUtil.getLogUtilsTag(AbstractSQLManager.class), sql);
			db.execSQL(sql);
			sql = "CREATE TRIGGER IF NOT EXISTS im_update_thread_on_insert2 AFTER INSERT ON "
					+ TABLES_NAME_IM_MESSAGE
					+ " "
					+ "BEGIN  "
					+ "UPDATE "
			...
			sql = "CREATE TRIGGER IF NOT EXISTS im_update_thread_on_update AFTER  UPDATE ON "
					+ TABLES_NAME_IM_MESSAGE
					+ " "
					+ "BEGIN   "
					+ "UPDATE "
			...
			sql = "CREATE TRIGGER IF NOT EXISTS thread_update_im_on_delete AFTER DELETE ON "
					+ TABLES_NAME_IM_SESSION
					+ " "
					+ "BEGIN DELETE FROM "
					+ TABLES_NAME_IM_MESSAGE
		...
		void createTriggerForSystemNotice(SQLiteDatabase db) {
			String sql = "CREATE TRIGGER IF NOT EXISTS system_update_thread_on_delete AFTER DELETE ON "
					...
					+ TABLES_NAME_IM_SESSION
					+ ".id = "
					+ SystemNoticeColumn.OWN_THREAD_ID
					+ " WHERE "
					+ SystemNoticeColumn.OWN_THREAD_ID
					+ " = old."
					+ SystemNoticeColumn.OWN_THREAD_ID
					...
					+ TABLES_NAME_SYSTEM_NOTICE
					+ " WHERE "
					+ SystemNoticeColumn.NOTICE_READ_STATUS
					+ " = 0 AND "
					+ SystemNoticeColumn.OWN_THREAD_ID
					+ " = old."
					+ SystemNoticeColumn.OWN_THREAD_ID
					...
					+ IThreadColumn.SNIPPET
					+ " FROM (SELECT "
					+ SystemNoticeColumn.NOTICE_DATECREATED
					+ ", "
					+ SystemNoticeColumn.NOTICE_VERIFYMSG
					+ " AS "
					+ IThreadColumn.SNIPPET
					+ ", "
					+ SystemNoticeColumn.OWN_THREAD_ID
					...
					+ IThreadColumn.SEND_STATUS
					+ " = "
					+ ECMessage.MessageStatus.SUCCESS.ordinal() + " ;   "
					// + "UPDATE " + TABLES_NAME_IM_SESSION + " SET " +
					// IThreadColumn.MESSAGE_TYPE + " = " +
					// com.speedtong.example.storage.GroupNoticeSqlManager.NOTICE_MSG_TYPE
					// + " ;   "
			...
			sql = "CREATE TRIGGER IF NOT EXISTS system_update_thread_on_insert AFTER INSERT ON "
					+ TABLES_NAME_SYSTEM_NOTICE
					+ " "
					+ "BEGIN  "
					+ "UPDATE "
					...
					+ IThreadColumn.MESSAGE_TYPE
					+ "="
					+ GroupNoticeSqlManager.NOTICE_MSG_TYPE
					+ " WHERE "
					+ TABLES_NAME_IM_SESSION
					+ ".id = new."
					+ SystemNoticeColumn.OWN_THREAD_ID
					...
					+ TABLES_NAME_IM_SESSION
					+ ".id = new."
					+ SystemNoticeColumn.OWN_THREAD_ID + ";  " + "END;";
			LogUtil.d(LogUtil.getLogUtilsTag(AbstractSQLManager.class), sql);
			db.execSQL(sql);

			sql = "CREATE TRIGGER IF NOT EXISTS system_update_thread_read_on_update AFTER  UPDATE OF "
					+ SystemNoticeColumn.NOTICE_READ_STATUS
					+ "  ON "
					+ TABLES_NAME_SYSTEM_NOTICE
					+ " "
			...
			sql = "CREATE TRIGGER IF NOT EXISTS system_update_thread_on_update AFTER  UPDATE ON "
					+ TABLES_NAME_SYSTEM_NOTICE
					+ " "
					+ "BEGIN   "
					+ "UPDATE "
			...
			sql = "CREATE TRIGGER IF NOT EXISTS thread_update_system_on_delete AFTER DELETE ON "
					+ TABLES_NAME_IM_SESSION
					+ " "
					+ "BEGIN DELETE FROM "
					+ TABLES_NAME_SYSTEM_NOTICE
		...
		public static void createImgInfoTable(SQLiteDatabase db) {
			String sql = "CREATE TABLE IF NOT EXISTS "
					...
					+ ImgInfoSqlManager.ImgInfoColumn.ID
					+ " INTEGER PRIMARY KEY AUTOINCREMENT, "
					...
					+ ImgInfoSqlManager.ImgInfoColumn.OTHER_STR + " TEXT ," 
					+ ImgInfoSqlManager.ImgInfoColumn.OTHER_STR2 + " TEXT ," 
					+ ImgInfoSqlManager.ImgInfoColumn.OTHER_INT + " INTEGER " 
					+ ")";
...
public static final String OTHER_STR = "other_str";
		...
		public static final String OTHER_STR2 = "other_str2";
	...
	class GroupColumn extends BaseColumn {
		...
		public static final String GROUP_isdefault = "isdefalut";
