...
//	}
    ...
    public static void checkContact(String contactid, String username) {
        LogUtil.d("ACTION_CONTACTS_CHANGE", "checkContact>>");
        if (!contactid.equals(CCPAppManager.getClientUser().getUserId())) {
            ECContacts ecR = ContactSqlManager.ckContact(contactid);
            if (TextUtils.isEmpty(ecR.getNickname())) {
            ...
            if (TextUtils.isEmpty(ecR.getHospital())) {
                ...
                intent.putExtra(IMNotifyReceiver.MESSAGE_SUB_CONTACTS_ID, contactid);
                LogUtil.d("ACTION_CONTACTS_CHANGE", "checkContact>> sendBroadcast  contactid>>" + contactid);
    ...
    public static long insertIMessage(ECMessage message, int boxType) {
        LogUtil.e("reuserdata_insert", "msg.getUserData()》》" + message.getUserData());
        long ownThreadId = 0;
        long row = 0L;
        try {
            if (!TextUtils.isEmpty(message.getSessionId())) {
                ...
                LogUtil.d("ACTION_CONTACTS_CHANGE", "insertIMessage>>");
                ...
                if (ownThreadId > 0) {
                    ...
                    if (boxType == IMESSENGER_BOX_TYPE_DRAFT) {
                        ...
                        try {
                            ...
                            if (dyMessage.getMessageType() == MessageType.SYSTEM.ordinal()) {
                                JSONObject jsonObject = null;
                                try {
                                    jsonObject = new JSONObject(dyMessage.getValue());
                                    String id = jsonObject.optString("id");
                                    values.put(IMessageColumn.OTHER_INT, jsonObject.optInt("p", 0));
                                    values.put(IMessageColumn.OTHER_STR2, id);
                                    if (jsonObject.has("data")) {
                                        JSONObject dataObj = jsonObject.optJSONObject("data");
                                        values.put(IMessageColumn.OTHER_STR, dataObj.toString());
                                    }
    ...
    public static ECMessage getLastHelperECMessage(String sessionId) {
        String sql = "select im_message.*  from im_message where exists ( select * from im_thread where im_thread.id = im_message.sid and im_thread.sessionId = "
                + sessionId + ")";
        Cursor cursor = getInstance().sqliteDB().rawQuery(sql, null);
        if (cursor != null && cursor.getCount() > 0) {
            if (cursor.moveToFirst()) {
    ...
    public static int updateIMessageDownload(ECMessage msg) {
        LogUtil.e("reuserdata_update", "msg.getUserData()》》" + msg.getUserData());
    ...
    public static ECMessage packageMessage(Cursor cursor) {
        ...
        String data =cursor.getString(cursor
                .getColumnIndexOrThrow(IMessageColumn.OTHER_STR));
     ...
     * @param msgId
     */
    public static void updateMsgReadStatus(String msgId) {
        if (getMsgReadStatus(msgId).equals("0")) {
            try {
                ContentValues values = new ContentValues();
                values.put(IMessageColumn.READ_STATUS, 1);
                getInstance().sqliteDB().update(DatabaseHelper.TABLES_NAME_IM_MESSAGE, values, "msgid = ?", new String[]{msgId});
