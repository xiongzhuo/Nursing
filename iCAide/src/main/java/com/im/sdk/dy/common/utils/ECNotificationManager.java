...
public class ECNotificationManager {
     ...
     * @param msg
     */
    public final void showCustomNewMessageNotification(Context context , String pushContent ,ECContacts contact ,ECMessage msg) {
    	String fromUserName=!TextUtils.isEmpty(contact.getRname())?contact.getRname():!TextUtils.isEmpty(contact.getNickname())?contact.getNickname():msg.getNickName();
    	String sessionId=msg.getSessionId();
    	int lastMsgType=msg.getType().ordinal();
    	
    	
    	LogUtil.w("showCustomNewMessageNotification",
        ...
        intent.putExtra("main_tab_index", "tab_msg_one");
...
//        ((NotificationManager)mContext.getSystemService(Context.NOTIFICATION_SERVICE)).notify(NOTIFY_ID_PUSHCONTENT, notification);
    ...
    public final void showCustomNewMessageNotification2(Context context , String pushContent ,ECContacts contact ,ECMessage msg) {
        String fromUserName=!TextUtils.isEmpty(contact.getRname())?contact.getRname():!TextUtils.isEmpty(contact.getNickname())?contact.getNickname():msg.getNickName();
        String sessionId=msg.getSessionId();
        int lastMsgType=msg.getType().ordinal();
        ...
        int sessionUnreadCount = ConversationSqlManager.getInstance().qureySessionUnreadCount();
        int allSessionUnreadCount = ConversationSqlManager.getInstance().qureyAllSessionUnreadCount();
        String contentTitle = msg.getNickName();
        ECTextMessageBody textBody = (ECTextMessageBody) msg.getBody();
        String contentText =textBody.getMessage();
        if(null==contentText){
            contentText="点击查看";
        }
    ...
    public final void showKickoffNotification(Context context , String kickofftext) {
    	  LogUtil.w("showKickoffNotification",
                  "showKickoffNotification kickofftext： " + kickofftext);
