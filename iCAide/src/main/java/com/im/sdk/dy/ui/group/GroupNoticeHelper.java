...
public class GroupNoticeHelper {
	...
	public static CharSequence getNoticeContent(String content) {
		...
		if (content.indexOf("<groupId>") != -1
			...
			DemoGroup ecGroup = GroupSqlManager.getDemoGroup(groupId);
	...
	public static void insertNoticeMessage(ECGroupNoticeMessage instanceMsg,
		...
		DemoGroupNotice demoGroupNotice = new DemoGroupNotice(instanceMsg
				.getType().ordinal());
		boolean show=true;
		String verifyMsg = "";
...
//			verifyMsg = inviteGroup(demoGroupNotice,invireMsg,show);
			 ...
			 GroupSqlManager.insertGroup(group, true,
			 false,invireMsg.isDiscuss()?true:false,false);
			 }
			...
			if (TextUtils.isEmpty(removeMsg.getNickName())) {
				ECContacts c = ContactSqlManager.getContact(removeMsg
						.getMember());
				if (c != null) {
			...
			if (TextUtils.isEmpty(instanceMsg.getGroupName())) {
				DemoGroup ecGroup = GroupSqlManager.getDemoGroup(instanceMsg
						.getGroupId());
				if (ecGroup != null) {
			...
			if (rInviteMsg.getConfirm() == 2) {
				if (rInviteMsg.isDiscuss()) {
					...
					verifyMsg = admin + "邀请 [" + rInviteMsg.getNickName()
							+ "]加入了讨论组";
				} else {

					verifyMsg = "群管理员" + admin + "邀请 ["
							+ rInviteMsg.getNickName() + "]加入了群组";
				}
			} else {
			...
			ECContacts contact = ContactSqlManager.getContact(modifyGroupMsg
					.getMember());
			if (contact == null) {
	...
	private static String inviteGroup(DemoGroupNotice demoGroupNotice,ECInviterMsg invireMsg,boolean show) {
		String message = invireMsg.getDeclared();
		String verifyMsg = "";

		if (null != message && message.contains(DyMessage.IM_MYDATA)) {
			DyMessage dyMessage = new DyMessage(message);
			if (dyMessage.getMessageType() == MessageType.GROUP.ordinal()) {

				JSONObject j = null;
				try {
					j = new JSONObject(dyMessage.getValue());
					int type = j.getInt("t");
					String name = j.getString("n");
					if (type == GroupCmdType.INVITEJOIN.ordinal()) {
						if (null == name || name.length() == 0) {
							verifyMsg = "您已加入群组 " + invireMsg.getGroupName();
						} else {
							verifyMsg = name + " 邀请您加入群组 "
									+ invireMsg.getGroupName();
						}
					
						
						demoGroupNotice.setSender(name);
						demoGroupNotice.setConfirm(invireMsg.getConfirm());
						demoGroupNotice.setDateCreate(invireMsg.getDateCreated());
						demoGroupNotice.setContent(verifyMsg);
						demoGroupNotice.setGroupId(invireMsg.getGroupId());
						demoGroupNotice.setGroupName(invireMsg.getGroupName());
					}else{
						show=false;
					}

				/*	ECGroup group = new ECGroup();
					group.setGroupId(invireMsg.getGroupId());
					group.setName(invireMsg.getGroupName());
					GroupSqlManager.insertGroup(group, true, false,
							invireMsg.isDiscuss() ? true : false,false);*/
