...
import com.im.sdk.dy.ui.group.NoticeSystemMessage;
...
public class GroupNoticeSqlManager extends AbstractSQLManager {
    ...
    public static long insertNoticeMsg(DemoGroupNotice notice) {
        ...
        if(notice != null) {
            ...
            if (ownThreadId == 0) {
                try {
                	LogUtil.d("ACTION_CONTACTS_CHANGE","insertNoticeMsg checkContact>>");
                    IMessageSqlManager.checkContact(notice.getSender(),notice.getNickName());
                    ECMessage message = ECMessage.createECMessage(ECMessage.Type.NONE);
